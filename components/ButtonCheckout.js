...
const ButtonCheckout = ({ priceId, mode = "payment" }) => {
  ...
  return (
    <Link
      href="/demo"
      className="btn hover:bg-[#013533] hover:text-white bg-white text-[#013533] btn-block group border border-[#013533]">
      {isLoading ? (
        <span className="loading loading-spinner loading-xs"></span>
      ) : (
        <svg
          className="w-5 h-5 group-hover:fill-white group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-200 fill-[#013533]"
          viewBox="0 0 375 509"
          fill="none"
